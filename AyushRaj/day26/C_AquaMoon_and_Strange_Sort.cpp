...
void chanel()
{
    ...
    cin >> n;
    map<int, vii> mpr;
    rep(i, 0, n) cin >> inp, mpr[inp].pb(i);
    ...
    for (auto it : mpr)
    {
        int odd = 0, even = 0;
        bool r = false;
        rep(i, 0, it.second.size()) if (bool(it.second[i] & 1)) odd++;
        even = it.second.size() - odd;

        if (bool(pos & 1) == 0 && !(even == odd || even == odd + 1)) //& use krne se 1ms ka fark par ra hai zada
            r = true;
        else if (bool(pos & 1) == 1 && !(odd == even || odd == even + 1))
            r = true;
